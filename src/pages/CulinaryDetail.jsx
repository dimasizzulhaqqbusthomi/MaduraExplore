...
import { MapPin, Heart, Utensils, Star, ShoppingBag, Store } from 'lucide-react';
import { AppContext } from '../context/AppContext';

export default function CulinaryDetail() {
  const { id } = useParams();
  const { culinaries, destinations, accommodations, toggleFavorite, isFavorite } = useContext(AppContext);

  const cul = culinaries.find(c => c.id === parseInt(id));
    ...
    return (
      <div className="section text-center" style={{ padding: '6rem 0', backgroundColor: 'var(--canvas)' }}>
        <div className="container card" style={{ maxWidth: '500px', margin: '0 auto', padding: '2rem', border: '1px solid var(--hairline)', borderRadius: '4px' }}>
          <h2 style={{ fontWeight: 300, marginBottom: '1rem' }}>Kuliner Tidak Ditemukan</h2>
          <p style={{ color: 'var(--ink-muted)', marginBottom: '1.5rem' }}>Maaf, kuliner tradisional yang Anda cari tidak tersedia dalam database kami.</p>
          <Link to="/culinary" className="btn btn-primary" style={{ borderRadius: '4px' }}>Kembali ke Kuliner</Link>
        </div>
      </div>
    );
  }

  const isFav = isFavorite(cul.id, 'culinary');

  // Coordinates mapping for recommended culinary spots
  const coordMap = {
    1: { lat: -7.1595, lng: 113.4795 }, // Pamekasan Alun-Alun (Sate)
    2: { lat: -7.2104, lng: 113.2505 }, // Sampang Kota (Bebek Songkem)
    3: { lat: -7.0395, lng: 112.7489 }, // Bangkalan Kota (Soto)
    4: { lat: -7.0094, lng: 113.8604 }, // Sumenep Kota (Kaldu Kokot)
    5: { lat: -7.2144, lng: 113.5412 }, // Pamekasan Pademawu (Lorjuk)
    6: { lat: -7.0142, lng: 112.7291 }  // Bangkalan (Tajin Sobih)
  };

  const coords = coordMap[cul.id] || { lat: -7.1068, lng: 113.4862 };
  ...
  const matchedTourism = destinations.filter(d => 
    cul.nearbyDestinations?.some(name => d.name.toLowerCase().includes(name.toLowerCase()) || name.toLowerCase().includes(d.name.toLowerCase()))
  );
    ...
    <div style={{ backgroundColor: 'var(--canvas)', minHeight: '100vh', padding: '3rem 0' }}>
      <div className="container">
        ...
        <div style={{ marginBottom: '1.5rem', fontSize: '0.85rem', color: 'var(--ink-muted)' }}>
          ...
          <Link to="/culinary" style={{ color: 'inherit', textDecoration: 'none' }}>Kuliner Madura</Link> &gt;{' '}
          <span style={{ color: 'var(--primary)', fontWeight: 600 }}>{cul.name}</span>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-1">
            <div className="card" style={{ border: '1px solid var(--hairline)', borderRadius: '4px', overflow: 'hidden', backgroundColor: 'var(--canvas)', marginBottom: '1.5rem' }}>
              <img src={cul.image} alt={cul.name} style={{ width: '100%', height: '240px', objectFit: 'cover' }} />
              <div style={{ padding: '1.25rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  <span style={{ fontSize: '0.8rem', color: 'var(--ink-muted)' }}>⭐ Rating Makanan</span>
                  <span style={{ fontWeight: 600, color: 'var(--ink)' }}>{cul.rating.toFixed(1)} / 5.0</span>
                </div>
                <button 
                  onClick={() => toggleFavorite(cul.id, 'culinary')}
                ...
                >
                  ...
                  {isFav ? 'Hapus Kuliner Favorit' : 'Simpan Kuliner Favorit'}
                </button>
              </div>
            </div>
          ...
          <div className="md:col-span-2">
            <div className="card" style={{ padding: '2rem', border: '1px solid var(--hairline)', borderRadius: '4px', backgroundColor: 'var(--canvas)' }}>
              <div style={{ display: 'flex', gap: '8px', marginBottom: '1rem' }}>
                <span className="badge" style={{ backgroundColor: 'var(--semantic-success)', color: 'white', fontSize: '0.75rem', fontWeight: 600, padding: '4px 8px', borderRadius: '4px' }}>
                  {cul.category.toUpperCase()}
                </span>
                <span className="badge" style={{ backgroundColor: 'var(--surface-1)', color: 'var(--ink)', fontSize: '0.75rem', fontWeight: 600, padding: '4px 8px', borderRadius: '4px', border: '1px solid var(--hairline)' }}>
                  Khas Kabupaten {cul.location}
                </span>
              </div>

              <h1 style={{ fontSize: '2.25rem', fontWeight: 300, color: 'var(--ink)', margin: '0 0 8px 0' }}>{cul.name}</h1>
              <p style={{ display: 'flex', alignItems: 'center', gap: '4px', color: 'var(--ink-muted)', fontSize: '0.9rem', margin: '0 0 1.5rem 0' }}>
                <Utensils size={16} /> Kuliner Tradisional Asli Pulau Madura
              </p>
              ...
              <div style={{ borderTop: '1px solid var(--hairline)', borderBottom: '1px solid var(--hairline)', padding: '1.25rem 0', margin: '1.5rem 0', display: 'flex', flexWrap: 'wrap', gap: '2rem' }}>
                <div>
                  <div style={{ fontSize: '0.75rem', color: 'var(--ink-muted)', fontWeight: 600, marginBottom: '4px' }}>ESTIMASI HARGA</div>
                  <div style={{ fontWeight: 600, color: 'var(--ink)', fontSize: '1.1rem' }}>{cul.priceRange}</div>
                </div>
                <div>
                  <div style={{ fontSize: '0.75rem', color: 'var(--ink-muted)', fontWeight: 600, marginBottom: '4px' }}>KABUPATEN UTAMA</div>
                  <div style={{ fontWeight: 600, color: 'var(--primary)', fontSize: '1.1rem' }}>{cul.location}</div>
                </div>
              </div>

              <h3 style={{ fontSize: '1.25rem', fontWeight: 300, color: 'var(--ink)', margin: '0 0 0.75rem 0' }}>Deskripsi Kuliner</h3>
              <p style={{ lineHeight: '1.6', color: 'var(--ink)', fontSize: '0.975rem', margin: '0 0 2rem 0', letterSpacing: '0.16px' }}>
                {cul.description}
              </p>

              {/* Recommended Places */}
              <h3 style={{ fontSize: '1.25rem', fontWeight: 300, color: 'var(--ink)', margin: '0 0 1rem 0' }}>Warung Rekomendasi Terkenal</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '2rem' }}>
                {cul.recommendedPlaces?.map((place, i) => (
                  <div key={i} className="card" style={{ padding: '1rem', border: '1px solid var(--hairline)', borderRadius: '4px', backgroundColor: 'var(--surface-1)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <Store size={18} style={{ color: 'var(--primary)' }} />
                    <span style={{ fontWeight: 600, fontSize: '0.95rem' }}>{place}</span>
                  </div>
                ))}
              </div>
              ...
              {matchedTourism.length > 0 ? (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1rem' }}>
                  {matchedTourism.map(dest => (
          ...
          <div style={{ padding: '0', marginTop: '2rem' }}>
            <div style={{ border: '1px solid var(--hairline)', borderRadius: '8px', padding: '1.5rem', backgroundColor: 'var(--canvas)' }}>
              <h2 style={{ fontSize: '1.25rem', fontWeight: 600, marginBottom: '1rem' }}>Penginapan Terdekat</h2>
              {(() => {
                const region = cul.location?.split(',').pop()?.trim() || cul.location;
                const nearbyAccommodations = (accommodations || []).filter(a => a.region === region || a.region === cul.location || cul.location?.includes(a.region)).slice(0, 3);
